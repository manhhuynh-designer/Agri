...
Hãy đối chiếu thông tin bài viết với kiến thức khoa học nông nghiệp hữu cơ và sinh học chính thống, chỉ rõ các phần có luận điểm cần lưu ý, kiểm chứng chéo với danh sách tài liệu trích dẫn ở chân bài viết và đưa ra kết luận khách quan về độ tin cậy.`;
    ...
    // 10. AI Summary Modal initialization
    let summaryModal = document.getElementById("ai-summary-modal");
    if (!summaryModal) {
      summaryModal = document.createElement("div");
      summaryModal.id = "ai-summary-modal";
      summaryModal.className = "verify-modal-overlay";
      summaryModal.innerHTML = `
        <div class="verify-modal-card" style="max-height:85vh;overflow:hidden;">
          <div class="verify-modal-header" style="border-bottom:1px solid var(--line);padding:16px 20px;display:flex;justify-content:space-between;align-items:center;">
            <div class="verify-header-title" style="display:flex;align-items:center;">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="width:20px;height:20px;color:var(--ember);margin-right:8px;vertical-align:middle;display:inline-block;">
                <path stroke-linecap="round" stroke-linejoin="round" d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z" />
                <polyline points="14 2 14 8 20 8" />
                <line x1="16" y1="13" x2="8" y2="13" />
                <line x1="16" y1="17" x2="8" y2="17" />
              </svg>
              <h4 style="margin:0;display:inline-block;vertical-align:middle;font-size:16px;font-family:'Archivo',sans-serif;">Tóm tắt bài viết bằng AI</h4>
            </div>
            <button class="summary-modal-close" aria-label="Đóng" style="background:none;border:none;color:var(--ash);font-size:28px;cursor:pointer;line-height:1;">&times;</button>
          </div>
          <div class="verify-modal-body" style="padding:20px;display:flex;flex-direction:column;gap:16px;overflow-y:auto;">
            <p style="font-size:13.5px;color:var(--ash-dim);margin:0;line-height:1.6;">
              Bạn có thể sao chép nhanh câu lệnh (prompt) dưới đây để gửi trực tiếp cho các mô hình AI lớn (ChatGPT, Gemini, Grok) yêu cầu tóm tắt ngắn gọn các luận điểm chính và giải pháp kỹ thuật của bài viết.
            </p>
            
            <div class="prompt-preview-container" style="border:1px solid var(--line);background-color:var(--bg);border-radius:8px;padding:12px;max-height:150px;overflow-y:auto;text-align:left;">
              <pre id="summary-prompt-preview-text" style="margin:0;font-family:'JetBrains Mono',monospace;font-size:12px;color:var(--ash-dim);white-space:pre-wrap;word-break:break-all;"></pre>
            </div>

            <div class="verify-action-buttons" style="display:flex;flex-direction:column;gap:10px;">
              <button id="copy-full-summary-btn" class="verify-action-btn primary" style="background-color:var(--ember);color:#ffffff;border:none;padding:12px;border-radius:6px;font-weight:600;font-size:13.5px;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;transition:var(--transition-smooth);">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" style="width:16px;height:16px;"><path stroke-linecap="round" stroke-linejoin="round" d="M8 5H6a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2v-1M8 5a2 2 0 002 2h2a2 2 0 002-2M8 5a2 2 0 012-2h2a2 2 0 012 2m0 0h2a2 2 0 012 2v3" /></svg>
                Sao chép Prompt & Nội dung bài viết (Tóm tắt đầy đủ)
              </button>
              <button id="copy-link-summary-btn" class="verify-action-btn secondary" style="background:none;border:1px solid var(--line);color:var(--ash);padding:10px;border-radius:6px;font-weight:600;font-size:13px;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;transition:var(--transition-smooth);">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" style="width:16px;height:16px;"><path stroke-linecap="round" stroke-linejoin="round" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" /></svg>
                Chỉ sao chép Prompt & Link bài viết
              </button>
            </div>
      ...
      summaryModal.querySelector(".summary-modal-close")?.addEventListener("click", closeSummaryModal);
    ...
    const openSummaryModal = () => {
      ...
      const previewText = `Hãy tóm tắt bài viết nông nghiệp dưới đây thành các ý chính ngắn gọn, súc tích và dễ áp dụng:
- Tiêu đề: ${articleTitle}
- Đường dẫn: ${currentUrl}

YÊU CẦU TÓM TẮT:
1. Tóm tắt cốt lõi bài viết trong 3-5 gạch đầu dòng.
2. Liệt kê các bước kỹ thuật / giải pháp thực hành chính.
3. Kết luận và các lưu ý/sai lầm cần tránh khi áp dụng.

NỘI DUNG BÀI VIẾT:
[... Toàn bộ nội dung chữ của bài viết ...]`;

      const previewEl = modal.querySelector("#summary-prompt-preview-text");
      if (previewEl) {
        previewEl.textContent = previewText;
      }

      const copyFullBtn = modal.querySelector("#copy-full-summary-btn");
      const copyLinkBtn = modal.querySelector("#copy-link-summary-btn");

      const handleCopyFullSummary = () => {
        const fullPrompt = `Hãy tóm tắt bài viết nông nghiệp dưới đây thành các ý chính ngắn gọn, súc tích và dễ áp dụng:
- Tiêu đề: ${articleTitle}
- Đường dẫn: ${currentUrl}
...
${articleBody}`;
        ...
        navigator.clipboard.writeText(fullPrompt)
          .then(() => showToast("Đã sao chép Prompt & Nội dung tóm tắt!"))
          .catch(() => showToast("Lỗi sao chép, vui lòng thử lại!"));
      };

      const handleCopyLinkSummary = () => {
        const linkPrompt = `Hãy truy cập đường dẫn dưới đây, đọc và tóm tắt bài viết nông nghiệp này thành các ý chính ngắn gọn, súc tích và dễ áp dụng:
- Tiêu đề: ${articleTitle}
- Đường dẫn: ${currentUrl}
...
3. Kết luận và các lưu ý/sai lầm cần tránh khi áp dụng.`;
        ...
        navigator.clipboard.writeText(linkPrompt)
          .then(() => showToast("Đã sao chép Prompt & Link tóm tắt!"))
          .catch(() => showToast("Lỗi sao chép, vui lòng thử lại!"));
      };

      copyFullBtn?.addEventListener("click", handleCopyFullSummary);
      copyLinkBtn?.addEventListener("click", handleCopyLinkSummary);
    };

    // Bind triggers
    const sidebarVerifyBtn = document.getElementById("ai-verify-sidebar-btn");
    const sidebarSummaryBtn = document.getElementById("ai-summary-sidebar-btn");
    ...
    const handleSummaryOpen = (e: Event) => {
      e.preventDefault();
      openSummaryModal();
    };

    sidebarVerifyBtn?.addEventListener("click", handleVerifyOpen);
    sidebarSummaryBtn?.addEventListener("click", handleSummaryOpen);
    ...
    return () => {
      ...
      sidebarSummaryBtn?.removeEventListener("click", handleSummaryOpen);
